...
local function get_ignored_files(tar_dir)
    ...
    if has_git() then
        ...
        local git_ls_results = RunShellCommand('git -C "' .. tar_dir ..
        ...
        for split_results in string.gmatch(git_ls_results, "([^\r\n]+)") do
            ...
            readout_results[#readout_results + 1] = (string.sub(split_results, -1) == "/" and
                                                        string.sub(split_results, 1, -2) or split_results)
        end
    end
...
local function refresh_view()
    ...
    if tree_view:GetView().Width < 20 or tree_view:GetView().Width > 30 then
...
function prompt_delete_at_cursor()
    ...
    micro.InfoBar():YNPrompt("Do you want to delete the " .. (scanlist[y].dirmsg ~= "" and "dir" or "file") .. ' "' ..
                                 scanlist[y].abspath .. '"? ', function(yes, canceled)
...
local function resize_tree(isDown)
    if tree_view == nil then
        return
    end

    local width = tree_view:GetView().Width

    if isDown then
        tree_view:ResizePane(width - 5)
    else
        tree_view:ResizePane(width + 5)
    end
...
function resize_up_tree()
    resize_tree(false)
end

function resize_down_tree()
    resize_tree(true)
end
...
function init()
    ...
    -- resize up view
    config.MakeCommand("resizeuptree", resize_up_tree, config.NoComplete)

    -- resize down view
    config.MakeCommand("resizedowntree", resize_down_tree, config.NoComplete)
    ...
    config.TryBindKey("F3", "command:resizeuptree", true)
    config.TryBindKey("F2", "command:resizedowntree", true)
